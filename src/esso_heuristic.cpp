...
std::unique_ptr<std::vector<std::vector<std::vector<double>>>> ComputeCostMatrix(
    problem_instance& p_instance, 
    const sfc_request& chain, 
    izlib::iz_path& path, 
    int ts) {
  ...
  for (int i = 0; i < path.size(); ++i) {
    ...
    cos[co].compute_embedding_cost(
        chain.cpu_reqs, chain.bandwidth, ts, temp_matrix);
...
std::unique_ptr<std::vector<int>> GenerateFirstFitSolution(
  ...
  for (int i = 0; i < path.size(); ++i) {
    ...
    for (int k = to_embed_start_idx; k < chain.vnf_count; ++k) {
      if (cost_matrix[to_embed_start_idx][k] == -1.0)
        break;
      to_embed_end_idx = k;
    }
...
bool IsValidAssignment(
    const problem_instance& p_instance,
  ...
  for (int i = 1; i < vnf_assignment.size(); ++i) {
    if (vnf_assignment[i] != current_co) {
      seg_end = i - 1;
      if (cost_matrices[current_co][seg_start][seg_end] == -1.0)
        return false;
...
std::unique_ptr<std::vector<int>> TabuSearch(
  ...
  auto initial_vnf_assignment = GenerateFirstFitSolution(
                                  p_instance, chain, path, cost_matrices);

  // First check if the initial solution is valid or not. If it is invalid then
  // declare embedding failed and return.
  if (!IsValidAssignment(p_instance, *initial_vnf_assignment.get(), cost_matrices, path))
    nullptr;
  ...
  double best_cost = GetAssignmentCost(
      current_vnf_assignment, cost_matrices);
  ...
  for (; iter < kMaxIterations; ++iter) {
    ...
    for (int j = 0; j < chain.vnf_count; ++j) {
      ...
      // move.
      if (tabu_set.find(std::pair<int, int>(j, neighbor->at(j))) != tabu_set.end())
        continue;
    ...
    if (best_neighbor.get() != nullptr) {
      if (best_neighbor_cost < best_cost) {
        ...
        tabu_timer[potential_tabu_move.first][potential_tabu_move.second] = kTabuPeriod;
        best_cost_update_timestamp = iter;
      }
    }

    // Update tabu list.
    for (auto tabu_it = tabu_set.begin(); tabu_it != tabu_set.end(); ++tabu_it) {
    ...
    if (iter - best_cost_update_timestamp > kMaxNonImprovedIterations)
      break;
...
std::unique_ptr<std::vector<int>> EssoHeuristic(
    problem_instance& p_instance, sfc_request& chain, int ts) {
  const int kNumShortestPaths = 3;
  auto embedding_path =
      GetPathWithMaxGreenEnergy(p_instance, chain, kNumShortestPaths, ts);
  auto cost_matrices = ComputeCostMatrix(p_instance, chain, *embedding_path.get(), ts);
  auto vnf_assignment = TabuSearch(p_instance, chain, *embedding_path.get(), *cost_matrices.get());
  return std::move(vnf_assignment);
}
